		...
		process.nextTick(function() {
			...
			if (!req.user) {
				User.findOne({
					'username' : username
						...
						function(err, user) {
							...
							// check to see if theres already a user with that
							// email
							if (user) {
				...
				User.findOne({
					...
					if (user) {
						return done(null, false, { 'message': 'The username is already taken.' });
						// Using 'loginMessage instead of signupMessage because
						// it's used by /connect/local'
					} else {
						var user = req.user;
				...
				// user is logged in and already has a local account. Ignore
				// signup. (You should log out before trying to create a new
				// account, user!)
